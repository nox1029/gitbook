...
main(int argc, char *argv[])
{
    ...
    if ((err = snd_pcm_hw_params_set_access(playback_handle, hw_params, SND_PCM_ACCESS_RW_ INTERLEAVED)) < 0)
    {
        fprintf(stderr, "cannot set access type (%s)\n",
                snd_strerror(err));
        exit(1);
    }
    if ((err = snd_pcm_hw_params_set_format(playback_handle, hw_params, SND_PCM_FORMAT_S16_ LE)) < 0)
    {
...
// Check that the microphone is enabled using alsamixer. Record by doing the following:
// alsa_capture hw:0 tmp.s16
// Play back by doing the following:
